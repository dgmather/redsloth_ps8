...
* using the delta method, this is asymototically distributed as:
$$\sqrt{n}(\frac{\hat{\beta}_1}{\hat{\beta}_2} ~ N(0, \sigma^2 R' Q_{xx}^{-1}R)$$
...
data_frame(co_X1=co_X1, 
           ...
           wald_q4 = as.numeric(t(x_boot2$boot_var_mean)*ratio*x_boot2$boot_var_mean)
) %>%
  bind_cols(x_boot2)
}
...
ratio_squared <- (sim_results[,1])^2
ratio_squared_avg <- colMeans(ratio_squared)
ratio_var <-  ratio_squared_avg[1] - (sim_avg[1])^2 
```  
# Simulations
...
Based on these, we expect to reject less than 5% of the time

# Actual wald distributions
``` {r, echo = FALSE}
...
#questions answered:
*question 1: expeect to reject above 5% with bootstrap Wald, maybe with mean of bootstrap Wald, probably not with sample Wald (order of variance from biggest to smallest)
*q2: see wtest_cval, wtest2_cval, crit--wtests have much bigger critical value than the actual asymptotic distribution
*q3: see wtest_reject, wtest2_reject--we reject too many!
*q4--I don't think this question makes any sense, but in any case see wald_q4
*q5--compare w_test to wtest_cval? I think this gives the right qualitative answer--bootstrap variance is higher so the sample statistic is more likely to land inside the bottom 95 percentiles than it would be on the actual distribution it comes from--but maybe our calculations are wrong, IDK
